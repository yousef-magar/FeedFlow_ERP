...
export default function HR() {
  ...
  const handleDownloadHRPDF = () => {
    ...
    `;
    const content = `
      <div class="watermark">${companyName || "FeedFlow ERP"}</div>
      <div class="header">
        <div class="header-right"><h1>${t("تقرير الموارد البشرية", "HR Report")}</h1><p class="sub">${periodLabel}</p></div>
        <div class="header-left">${companyLogo ? `<img src="${companyLogo}" style="max-height:50px;margin-bottom:4px" alt=""/>` : ""}<div class="company">${companyName || "FeedFlow ERP"}</div>${companyAddress ? `<div>${companyAddress}</div>` : ""}<div style="margin-top:2px">${t("تاريخ الإنشاء", "Generated")}: ${nowStr}</div></div>
      </div>
      ...
      <div class="footer"><p><span class="footer-logo">${companyName || "FeedFlow ERP"}</span> — ${t("جميع الحقوق محفوظة", "All rights reserved")} © ${new Date().getFullYear()}</p><p style="margin-top:2px">${t("تم إنشاؤه بواسطة", "Generated by")} FeedFlow ERP — ${nowStr}</p></div>
    `;
                      ...
                      <Card className="border-border/60 overflow-hidden">
                        <div className="flex flex-wrap items-center gap-2 p-2.5">
                          ...
                          <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                            onClick={() => removeShift(sh.id)} className="text-muted-foreground hover:text-destructive shrink-0">
                            <X className="w-4 h-4" />
                          </motion.button>
                        </div>
                      </Card>
