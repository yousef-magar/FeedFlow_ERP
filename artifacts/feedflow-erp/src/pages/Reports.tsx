  ...
  const handleDownload = () => {
    ...
    const content = `
      ...
      <div class="header">
        ...
        <div class="header-left">${companyLogo ? `<img src="${companyLogo}" style="max-height:45px;margin-bottom:3px" alt=""/>` : ""}<div class="company">${companyName || "FeedFlow ERP"}</div>${companyAddress ? `<div>${companyAddress}</div>` : ""}<div style="margin-top:2px">${t("تاريخ الإنشاء", "Generated")}: ${nowStr}</div></div>
      </div>
